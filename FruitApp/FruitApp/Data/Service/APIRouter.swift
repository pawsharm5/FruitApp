...
final class APIRouter<EndPoint: EndPointType> {
    ...
    private func decodingTask<T: Decodable>(with request: URLRequest, decodingType: T.Type, completionHandler completion: @escaping JSONTaskCompletionHandler) -> URLSessionDataTask {
        let session = URLSession(configuration: .default, delegate: SSLPinningDelegateManager(isSSLPinningEnabled: true), delegateQueue: nil)
        let task = session.dataTask(with: request) { data, response, error in
            ...
            do {
                ...
                completion(EmptyData().dataRepresentation(), APIError.jsonDecodingFailure)
            }
        }
        return task
    }
